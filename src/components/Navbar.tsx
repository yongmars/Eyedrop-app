...
export default function Navbar() {
  ...
  const getTabClass = (path: string) => {
    ...
    return `flex flex-col items-center justify-center w-full h-full transition-colors ${
      isActive
        ? "text-blue-500 bg-blue-50/30 dark:bg-slate-700/50"
        : "text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-slate-700/30"
    }`;
  };

  return (
    <nav className="absolute bottom-0 w-full h-16 bg-white dark:bg-slate-800 border-t border-blue-100 dark:border-slate-700 flex items-center justify-around z-50 animate-slide-in-fast">
      ...
      <Link href="/calendar" className={getTabClass("/calendar")}>
        ...
        <span className="text-[10px] mt-1 font-medium">カレンダー</span>
      </Link>
      <Link href="/camera" className={getTabClass("/camera")}>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/><circle cx="12" cy="13" r="3"/></svg>
        <span className="text-[10px] mt-1 font-medium">カメラ</span>
      </Link>
